...
mrms21 <- read_csv('mrms_21.csv') %>%
  ...
  mutate(datetime  = datetime - (2 * 60 * 60)) #should only be -1, figure out time correction
...
mrms_metrics$color_var <- ifelse(substr(mrms_metrics$site, 1, 1) == "m", "m_sites", "other_sites")
...
ggsave("P_boxplot.png", plot = p_boxplot, width = 8, height = 6, dpi = 300)
...
uno_v_uno <- left_join(mrms_mi60, tb_mi60_comp, by = c('date', 'site', 'starttime_floor'))
...
mrms_response$color_var <- ifelse(substr(mrms_response$site, 1, 1) == "m", "m_sites", "other_sites")
...
comp1 <- mrms_response %>%
  ggplot(aes(MI60, Stage_range/10, color = site, shape = site)) +
  geom_point() +
  labs(x='MI60', y = 'Stage rise (cm)') +
  theme(plot.title = element_text(size = 16, face = "bold"),
        axis.title = element_text(size = 14),
        axis.text = element_text(size = 12),
        panel.grid.major = element_blank(),
...
ggsave("compare.png", plot = comp2, width = 8, height = 6, dpi = 300)

ggplotly(ggplot(mrms_response, aes(Stage_range/10)) + geom_histogram(binwidth=2.5))

ggplotly(ggplot(mrms_response, aes(Stage_range)) + geom_density(color = "blue", fill = "lightblue", alpha = 0.5))
...
me_glm <- lm(Stage_range ~ q_cms + doy +P + duration_hr + MI60 + bed_change_mm, data = me_response2)

summary(me_glm)
summary(me_glm)$r.squared


mm_glm <- lm(Stage_range ~ q_cms + doy +P + duration_hr + MI60 + bed_change_mm, data = mm_response2)

summary(mm_glm)
summary(mm_glm)$r.squared

mw_glm <- lm(Stage_range ~ q_cms + doy +P + duration_hr + MI60 + bed_change_mm, data = mw_response2)

summary(mw_glm)
summary(mw_glm)$r.squared

ue_glm <- lm(Stage_range ~ q_cms + doy +P + duration_hr + MI60 + bed_change_mm, data = ue_response2)

summary(ue_glm)
summary(ue_glm)$r.squared

um_glm <- lm(Stage_range ~ q_cms + doy +P + duration_hr + MI60 + bed_change_mm, data = um_response2)

summary(um_glm)
summary(um_glm)$r.squared

uw_glm <- lm(Stage_range ~ q_cms + doy +P + duration_hr + MI60 + bed_change_mm, data = uw_response2)
